...
class RBM:
    ...
    def train(
        ...
        for epoch in range(n_epochs):
            ...
            if (epoch % print_each == 0 or epoch == n_epochs - 1) * verbose:
                tqdm.write(
                    f"Epoch {epoch}. Reconstruction error: {quadratic_error: .4f}."
                )
            ...
            if save_each and (epoch % save_each == 0):
                if save_path is None:
                    raise ValueError(
                        "`save_path` is required when `save_each` is not None."
                    )
    ...
    def load_weights(
        self, path: str, dict_weights: Optional[Dict[str, np.ndarray]] = None
    ) -> None:
        """
